...
//	@ManagedProperty(value = "#{GeneraMarcoLogicoDefaultviewView.generaMarcoLogico.tipoProyecto}")
	...
	private transient caf.war.fcmpcPortales.wsclient.sfcmpc.services.completadortarea_wsd.CompletadorTarea completadorTarea = null;
	private static final String[][] COMPLETADORTAREA_PROPERTY_BINDINGS = new String[][] {
		{"#{CompletadorTarea.authCredentials.authenticationMethod}", "1"},
		{"#{CompletadorTarea.authCredentials.requiresAuth}", "true"},
		{"#{CompletadorTarea.autoRefresh}", "false"},
	};
	private static final String[][] ACTIONACTUALIZADATA_PROPERTY_BINDINGS = new String[][] {
		{"#{GeneraMarcoLogicoDefaultviewView.completadorTarea.parameters.completadorTarea.completadorTarea.entrada.publicaMarcoV1.marcoLogicoV1}", "#{GeneraMarcoLogicoDefaultviewView.generaMLogico_1.parameters.generaMLogico_1.generaMLogico_1.marcoLogicoV1}"},
	};
	...
	public String initialize() {
		try {
			...
			if (this.getGeneraMarcoLogico().getIndicaDor()!=null)
			{	
			  this.setIndicador(this.getGeneraMarcoLogico().getIndicaDor());
			}
	...
	}else
	{
		 ...
		 this.getCompletadorTarea().getParameters().getCompletadorTarea().getCompletadorTarea().getEntrada().setTaskid(this.getPropertyTaskId());
		 this.actionActualizaData();
		 this.getCompletadorTarea().refresh();
		 this.getGeneraMLogico_1().getResult().getGeneraMLogico_1Response().getSalida().setMsg(this.getCompletadorTarea().getResult().getCompletadorTareaResponse().getSalida().getMsg());
	...
	public caf.war.fcmpcPortales.wsclient.sfcmpc.services.completadortarea_wsd.CompletadorTarea getCompletadorTarea()  {
		if (completadorTarea == null) {
		    completadorTarea = (caf.war.fcmpcPortales.wsclient.sfcmpc.services.completadortarea_wsd.CompletadorTarea)resolveExpression("#{CompletadorTarea}");
		}
	
	    resolveDataBinding(COMPLETADORTAREA_PROPERTY_BINDINGS, completadorTarea, "completadorTarea", false, false);
		return completadorTarea;
	}

	public String actionActualizaData() {
	    resolveDataBinding(ACTIONACTUALIZADATA_PROPERTY_BINDINGS, this, "actionActualizaData.this", true, false);
		return null;
	}
	 ...
	 * Getter method for the control with id='defaultForm:asyncTable:supuestos1'
	 */
	public com.webmethods.caf.faces.component.panel.BaseHideablePanel getSupuestos1_2()  {
		return (com.webmethods.caf.faces.component.panel.BaseHideablePanel)findComponentInRoot("defaultForm:asyncTable:supuestos1");
	}
